...
namespace apriltag
{
	...
	matd_t* fix_pose_ambiguities(matd_t** v, matd_t** p, matd_t* t, matd_t* R, int n_points)
	{
		...
		matd_t* R_t = matd_create_data(3, 3, tmp_R_t);
		matd_destroy(R_t_1);
		matd_destroy(R_t_2);
		matd_destroy(R_t_3);
		...
		const double tmp_R_gamma[] = {
			cos_gamma, -sin_gamma, 0,
			sin_gamma, cos_gamma, 0,
			0, 0, 1 };
		matd_t* R_gamma = matd_create_data(3, 3, tmp_R_gamma);
		...
		const double tmp_M1[] = {
			...
			0, 0, 0,
			-2, 0, 0 };
		matd_t* M1 = matd_create_data(3, 3, tmp_M1);
		const double tmp_M2[] = {
			-1, 0, 0,
			0, 1, 0,
			0, 0, -1 };
		matd_t* M2 = matd_create_data(3, 3, tmp_M2);
		...
		double roots[4];
		int n_roots;
		double tmp_p[] = { p0, p1, p2, p3, p4 };
		solve_poly_approx(tmp_p, 4, roots, &n_roots);
	...
	{
		double scale = info->tagsize / 2.0;
		const double tmp_data_1[] = { -scale, scale, 0 };
		const double tmp_data_2[] = { scale, scale, 0 };
		const double tmp_data_3[] = { scale, -scale, 0 };
		const double tmp_data_4[] = { -scale, -scale, 0 };
		matd_t* p[4] = {
			matd_create_data(3, 1, tmp_data_1),
			matd_create_data(3, 1, tmp_data_2),
			matd_create_data(3, 1, tmp_data_3),
			matd_create_data(3, 1, tmp_data_4) };
		matd_t* v[4];
		for (int i = 0; i < 4; i++)
		{
			const double tmp[] = { (info->det->p[i][0] - info->cx) / info->fx, (info->det->p[i][1] - info->cy) / info->fy, 1 };
			v[i] = matd_create_data(3, 1, tmp);
		}
